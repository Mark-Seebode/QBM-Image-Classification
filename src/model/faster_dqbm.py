...
class Disc_QBM():
        ...
        if (self.solver_string == "Advantage_system4.1" or self.solver_string == "Advantage_system7.1") and self.parallelize\
            ...
            if not self.restricted:
                ...
                self.parallel_embeddings_clamped = self.load_embeddings(
                    f"src/embeddings/advantage_4_1/integer/embeddings_clamped_{self.n_hidden_nodes}.pkl")
            ...
            self.parallel_embeddings_unclamped = self.load_embeddings(
                f"src/embeddings/advantage_4_1/integer/embeddings_unclamped_{self.n_hidden_nodes}.pkl")
            print("Embeddings loaded.")

            # self.subgraphs = self.calcualte_parallel_subgraphs()
            self.subgraphs = self.load_subgraphs(
                f"src/embeddings/advantage_4_1/integer/subgraphs_{len(self.parallel_embeddings_unclamped)}.pkl")
    ...
    def create_qubo_matrix_from(self, input_vector: np.ndarray, label: np.ndarray = None):
        ...
        #                                    hb4
            ...
            weight_output_to_hidden = np.pad(weight_output_to_hidden,
                                             ...
                                             constant_values=0)
            # weight_output_to_hidden.reshape((1, self.n_hidden_nodes))
    ...
    def get_qa_samples(self, qubo_as_bqm, sample_count, label = None):
        ...
        answer = self.run_qa_sampling(embedded_q, this_embedding, qubo_as_bqm, sample_count)
    ...
    def get_parallel_qa_samples(self, qubo_matrix, label=None):
        ...
        #print("\n\n", combined_embedding)
        #raise Exception("stop")
        num_parallel_embeddings = len(self.parallel_embeddings_clamped)
        num_tasks = int(self.sample_count / num_parallel_embeddings)
        rest_of_tasks = self.sample_count % num_parallel_embeddings

        answer = self.run_qa_sampling(combined_embedded_bqm, combined_embedding, combined_source_qubo_bqm,
                                      num_tasks) # sample_count ist immer in 10ner Schritten
        samples = list(answer.samples())
        #print("\n\nsamples",samples)
        ...
        #raise Exception("stop")
        if rest_of_tasks != 0:
            single_sample = self.get_qa_samples(bqm, int(rest_of_tasks), label)
            splitted_samples += single_sample
    ...
    def get_samples(self, input_vector, label=None):
        ...
        if self.solver_string == "SA":
            ...
            assert qubo_assertion.shape[0] == qubo_assertion.shape[1], "QUBO matrix must be square"
                ...
                if not self.parallelize:
                    if self.restricted:
                        if label is not None:
                            ...
                            samples = self.get_qa_samples(qubo_as_bqm, self.sample_count, label)
                    else:
                        samples = self.get_qa_samples(qubo_as_bqm, self.sample_count, label)
                else:
    ...
    def get_average_configuration_batch(self, samples_batch: list, x_batch, y_batch=None):
        ...
        avgs_biases_hidden = np.zeros(self.biases_hidden.shape)
        avgs_biases_output = np.zeros(self.biases_output.shape)
        avgs_weights_visible_to_hidden = np.zeros(
            self.weights_all_visible_to_hidden.shape)
        avgs_weights_visible_to_output = np.zeros(
            self.weights_clamped_visible_to_output.shape)
    ...
    def calcualte_parallel_subgraphs(self, num_partitions=10):
        ...
        with open(f"src/embeddings/advantage_4_1/integer/subgraphs_{num_partitions}.pkl", "wb") as f:
    ...
    def get_samples_batch(self, input_batch, label_batch=None):
        samples_all = []
        for i in tqdm(range(len(input_batch)), desc="Getting samples of current batch", ncols=80, leave=False):
            x = input_batch[i]
    ...
    def train_for_one_iteration(self, x_batch, y_batch, learning_rate, nll):
        ...
        #for i in tqdm(range(len(x_batch)), desc="Sampling current batch", ncols=80, leave=False):
        samples_clamped_batch= self.get_samples_batch(x_batch, label_batch=y_batch)
        ...
        samples_unclamped_batch = self.get_samples_batch(x_batch)
        ...
        avgs_bias_hidden_clamped, avgs_bias_output_clamped, avgs_weights_visible_to_hidden_clamped, avgs_weights_visible_to_output_clamped, avgs_weights_output_output_clamped, avgs_weights_hidden_hidden_clamped = self.get_average_configuration_batch(
            samples_clamped_batch, x_batch, y_batch)

        avgs_bias_hidden_unclamped, avgs_bias_output_unclamped, avgs_weights_visible_to_hidden_unclamped, avgs_weights_visible_to_output_unclamped, avgs_weights_output_output_unclamped, avgs_weights_hidden_hidden_unclamped = self.get_average_configuration_batch(
            samples_unclamped_batch, x_batch)
        ...
        avg_batch_loss = self.compute_nll(y_batch, samples_unclamped_batch, nll)
    ...
    def save_weights(self, title, path="out"):
        ...
        #                     b_v=self.biases_visible, b_h=self.biases_hidden, lat=self.weights_clamped_visible_to_output)
    ...
    def find_embedding_with_client(self, bqm, save, label = None):
        ...
        if save:
            ...
            path.mkdir(mode=0o770, exist_ok=True)
            plt.savefig("embeddings/sequential_unclamped_embedding.png", transparent=True)
